...
public class Grid {
    ...
    public static void paintGrid(Canvas canvas, int height, int width) {
    ...
    public static void doOneStep() {
        ...
        for (GridPoint point : points) {
            GridPoint reusedTempPointForVar = new GridPoint(point.getXPoint() - 1, point.getYPoint() - 1);
            if (shouldAddPoint(reusedTempPointForVar, true)) {
                if (!isAlreadyAdded(newPoints, reusedTempPointForVar)) {
                    newPoints.add(reusedTempPointForVar);
                }
            }
            reusedTempPointForVar = new GridPoint(point.getXPoint() - 1, point.getYPoint());
            if (shouldAddPoint(reusedTempPointForVar, true)) {
                if (!isAlreadyAdded(newPoints, reusedTempPointForVar)) {
                    newPoints.add(reusedTempPointForVar);
                }
            }
            reusedTempPointForVar = new GridPoint(point.getXPoint() - 1, point.getYPoint() + 1);
            if (shouldAddPoint(reusedTempPointForVar, true)) {
                if (!isAlreadyAdded(newPoints, reusedTempPointForVar)) {
                    newPoints.add(reusedTempPointForVar);
                }
            }
            reusedTempPointForVar = new GridPoint(point.getXPoint(), point.getYPoint() - 1);
            if (shouldAddPoint(reusedTempPointForVar, true)) {
                if (!isAlreadyAdded(newPoints, reusedTempPointForVar)) {
                    newPoints.add(reusedTempPointForVar);
                }
